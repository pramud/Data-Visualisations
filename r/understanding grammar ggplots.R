...
# The dia_plot object has been created for you. Update dia_plot so that this time it contains all the functions to make a scatter plot instead of just the data and aes layers. Your command should start with dia_plot <- dia_plot + .... For this instruction, use geom_point() for the geom layer. Set alpha to 0.2. Using +, plot the dia_plot object with a geom_smooth() layer on top. You don't want any error shading, which can be achieved by setting the se argument in geom_smooth() to FALSE. In order to plot the figure, your command should start with dia_plot + ... here. Modify the geom_smooth() function from the previous instruction so that it contains aes() and map clarity to the col argument.
